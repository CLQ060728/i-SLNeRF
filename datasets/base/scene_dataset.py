...
class SceneDataset(abc.ABC):
    ...
    def get_aabb(self, aabb_type: str, aabb: List) -> Tensor:
        if self.lidar_source is not None:
            ...
            if aabb_type == "front":
                ...
                aabb = self.pixel_source.get_aabb_auto()
            elif aabb_type == "specified":
                aabb = self.pixel_source.get_aabb_specified(aabb)
            else:
                raise ValueError(f"Unknown aabb type: {aabb_type}!")
